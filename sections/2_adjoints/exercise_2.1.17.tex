	...
	\item
		...
		\begin{casedistinction}
			...
			\item
				...
				It follows that the two morphisms~$∇(B)(f' ∘ f)$ and~$∇(B)(f) ∘ ∇(B)(f')$ are equal, since they both go from~$∇(B)(A'')$ to~$T(\cat{B})$.

		\end{casedistinction}
		We have in either case that~$∇(B)(f' ∘ f) = ∇(B)(f) ∘ ∇(B)(f')$.

\end{itemize}
With this, we have shown that the assignment~$∇(B)$ is indeed a contravariant functor from~$\cat{A}$ to~$\cat{B}$.
...
To show that is it surjective, let~$g$ be a morphism from~$Γ(F)$ to~$B$, i.e., a morphism from~$F(T(\cat{A}))$ to~$B$.
