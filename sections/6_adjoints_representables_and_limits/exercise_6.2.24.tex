...
\[
	...
	Y(A)
\]
that is given for every element~$x$ of~$X(A)$ on the~\nth{$x$} summand of \eqref{decomposition into coproduct} by the inclusion map from~$α_A^{-1}(x)$ to~$Y(A)$.
We denote this inclusion map by
\[
	i_{(Y, α), A, x} \colon α_A^{-1}(x) \to Y(A) \,.
\]

These bijections~$ε_{(Y, α), A}$ assemble altogether into a transformation
\[
